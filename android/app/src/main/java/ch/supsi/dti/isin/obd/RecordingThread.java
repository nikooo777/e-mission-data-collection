...
import ch.supsi.dti.isin.obd.output.OutputFile;
...
public class RecordingThread implements Runnable {
    ...
    public void run() {
        ...
        while(RUN){
            ...
            if(!serv.getCarManager().isConnected()){
               // Toast.makeText(serv.getApplicationContext(), "Car is not connected", Toast.LENGTH_LONG).show();
                ...
                try {
                    ...
                    e.printStackTrace();
                    Toast.makeText(serv.getApplicationContext(), "ConnectionException: Car is not connected", Toast.LENGTH_LONG).show();
